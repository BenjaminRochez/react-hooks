...
We used React.useEffect to sync the state of our app with the localstorage. We then lazyloaded it by calling a function in the useState. After that, we learned about the useEffect dependency that basically is an array of variables that useEffect will be watching and will re-render only if this var is altered.
We were introduced to the custom hooks and refactored the code to abstract the logic of the localstorage management inside a hook. /!\ convention: write a hook with use at the beginning of the name : useCustomHook, useSomethingHook; Hooks are hooks because they are function that contains hooks.
## Background
