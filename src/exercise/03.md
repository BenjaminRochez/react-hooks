...
We learned how to lift the state, meaning to update our code so the state was managed by the parent instead of the child. We then made the reverse process, we took the state managed by the parent and placed it inside the child component. This is called collocate the state.
